...
let nameClassStateSort = 'title_a'; 
let this._columnConfig = {
    name: {
      title: 'Название',
      issortable: true,
      issearchable: true,
...
let [...this._statePhoneInTable] = phones;

render();
function render() { //рисует таблицу
    ...
    Object.keys(this._columnConfig).map((conf) => string += `<th
        ...
        data-name="${conf}"
        class="${nameClassStateSort}"
    >${this._columnConfig[conf].title}</th>` );
    string += `</tr>`;

    Object.keys(this._statePhoneInTable).map(elem => {
        string += '<tr><td><input type="checkbox" data-checkbox=""></td>';
        Object.keys(this._columnConfig).map(el => string += `<td>${this._statePhoneInTable[elem][el]}</td>`);
        string += '</tr>';
    });
